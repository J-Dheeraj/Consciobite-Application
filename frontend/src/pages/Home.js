                ...
                {suggestions.map((p) => (
                  ...
                  >
                    ...
                    <div style={{ flex: 1, minWidth: 0 }}>
                      ...
                      <div style={{ fontSize: "0.78rem", color: isDark ? "#7a9a7e" : "#888" }}>{p.brand} {"\u00B7"} {p.category}</div>
                    </div>
                    <div style={{ fontSize: "0.75rem", color: isDark ? "#7a9a7e" : "#aaa" }}>{p.greenGrade.totalEmissions} kg</div>
                  </button>
                ))}
              </div>
            )}
      ...
      <div style={{ maxWidth: 720, margin: "0 auto", padding: "0 20px 40px" }}>
        ...
        <div style={{ display: "flex", gap: 8, overflowX: "auto", padding: "20px 0 16px", WebkitOverflowScrolling: "touch" }}>
          {CATEGORIES.map((c) => (
            <button key={c.key} onClick={() => setCategory(c.key)} style={{
              padding: "8px 16px", borderRadius: 24, border: category === c.key ? "2px solid #2d6a4f" : "2px solid " + (isDark ? "#2d4a35" : "#e0e0e0"),
        ...
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16 }}>
          ...
          <select value={sort} onChange={(e) => setSort(e.target.value)} aria-label="Sort products"
            style={{ padding: "8px 12px", borderRadius: 10, border: "1px solid " + (isDark ? "#2d4a35" : "#e0e0e0"), fontSize: "0.85rem", color: isDark ? "#b0c4b1" : "#555" }}>
        ...
        {error && (
          <div style={{ color: "#e63946", marginBottom: 12, padding: 20, background: isDark ? "#2a1519" : "#fef2f2", borderRadius: 12, border: "1px solid #fecaca", animation: "fadeInUp 0.3s ease" }}>
        ...
        {!loading && !error && (
          <>
            <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
              {products.length === 0 && (
                <div style={{ textAlign: "center", padding: 40, color: "#888", background: isDark ? "#1c2e22" : "#fff", borderRadius: 14, boxShadow: "0 1px 4px rgba(27,67,50,0.06)" }}>
            ...
            {pagination && pagination.totalPages > 1 && (
              <div style={{ display: "flex", justifyContent: "center", alignItems: "center", gap: 12, marginTop: 28 }}>
                <button onClick={() => setPage((p) => Math.max(1, p - 1))} disabled={!pagination.hasPrev} aria-label="Previous page"
                  style={{ padding: "10px 20px", borderRadius: 10, border: "none", background: pagination.hasPrev ? "#2d6a4f" : (isDark ? "#2d4a35" : "#e8e8e8"), color: pagination.hasPrev ? "#fff" : "#aaa", cursor: pagination.hasPrev ? "pointer" : "not-allowed", fontWeight: 600, fontSize: "0.85rem", transition: "all 0.2s ease" }}>
                  Previous
                </button>
                <span style={{ fontSize: "0.88rem", color: isDark ? "#7a9a7e" : "#666", fontWeight: 500 }}>
                  {pagination.page} / {pagination.totalPages}
                </span>
                <button onClick={() => setPage((p) => p + 1)} disabled={!pagination.hasNext} aria-label="Next page"
                  style={{ padding: "10px 20px", borderRadius: 10, border: "none", background: pagination.hasNext ? "#2d6a4f" : (isDark ? "#2d4a35" : "#e8e8e8"), color: pagination.hasNext ? "#fff" : "#aaa", cursor: pagination.hasNext ? "pointer" : "not-allowed", fontWeight: 600, fontSize: "0.85rem", transition: "all 0.2s ease" }}>
                  Next
                </button>
              </div>
            )}
