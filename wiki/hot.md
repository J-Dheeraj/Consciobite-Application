...
**Carbon CSV export (PR #39, branch `claude/nifty-goodall-juod2y`):** `GET /api/carbon/export` — auth-required, RFC 4180 CSV of the user's carbon logs with Content-Disposition. `downloadCarbonExport()` in the frontend carbon service (raw fetch for blob); "Export CSV" button on the carbon page, hidden when no logs.
