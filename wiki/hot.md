...
**Last updated:** 2026-07-18 after passport test session.
...
**Current test status:** 163 backend tests passing (7 test suites). Frontend builds 566 static pages (16 routes + 550 product pages).

**Digital Product Passport API (2026-06-07, landed in main):** B2B endpoints at `/api/v1/` — `GET /passport/:productId`, `POST /portfolio/score`, `GET /audit/:productId`. EU ESPR and SGX Scope 3 reporting. `METHODOLOGY.md` added to repo root with GreenGrade v3.0 full technical spec. `ApiReadyGate` component added for cold start UX on free Render tier.

**Active branch:** `claude/nifty-goodall-fcberl` — passport integration test coverage added (26 new tests).
