...
**Last updated:** 2026-06-04 after auth role, admin nav, and frontend test session.
...
**Current test status:** 137 backend tests passing. 9 frontend smoke tests passing (Spinner, PageHero, GradeBadge). Frontend builds 566 static pages (16 routes + 550 product pages).

**Auth invariant (2026-06-04):** `role` is now included in JWT payload, login response, and `/api/auth/me`. Frontend uses `user.role === 'admin'` to gate admin nav links and admin pages. Footer "Products" link fixed (was "/" now "/products").

**Active branch:** `claude/nifty-goodall-DY7DC` — pushed. Contains governance frontend (PR #31, ahead of main) + 2026-06-04 auth/nav/test fixes.
