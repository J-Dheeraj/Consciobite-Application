...
**Last updated:** 2026-08-20 — all PRs merged (including #45 community evidence submission, #48 trailing-slash fix); admin evidence review UI landed on branch `claude/dreamy-dirac-5syynd`.
...
**Open PRs:** none — all PRs through #48 are merged into main. Branch `claude/dreamy-dirac-5syynd` has admin landing page + evidence review UI (commit `6c5f37e`) ready to PR.
...
**Admin panel (2026-08-20, commit `6c5f37e`):** `/admin` hub page links to Score Audit, Manufacturers, Evidence Review. `/admin/evidence` shows pending community evidence submissions; admin can expand each item, add notes, and approve/reject. `GET /api/auth/me` now returns `role` field; Navbar shows "Admin" link only when `user.role === "admin"`.
