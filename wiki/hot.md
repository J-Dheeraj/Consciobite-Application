...
**Last updated:** 2026-07-10 after recommendations + reviews integration session.
...
**Current test status:** 142 backend tests passing (5 new for recommendations endpoint). Frontend builds 566 static pages (16 routes + 550 product pages).

**Active branch:** `claude/dreamy-dirac-xcy6m0` — new feature branch from main.

**Recommendations + Reviews (2026-07-10):** `GET /api/products/:id/recommendations` endpoint added to `backend/src/routes/products.js`. Returns top 4 same-category products by GreenGrade score (cross-category fallback when pool is small). Returns minimal fields (id, name, brand, category, greenGrade.{score,color,grade}). `ProductDetailClient.js` now uses `fetchRecommendations()` via React Query to show a horizontally-scrollable "Similar Products" card. `ReviewSection` component (previously built but unused) is now rendered below similar products on the product detail page. Branch: `claude/dreamy-dirac-xcy6m0`.
