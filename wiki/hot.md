...
**Last updated:** 2026-06-24 after catch-up audit (no open PRs; backed-filled wiki for two unlogged sessions; health check clean).
...
**Current test status (2026-06-24):** 137 backend tests passing (6 suites), `npm run lint` clean, frontend builds 566 static pages (16 routes + 550 product pages). All green — no open PRs, `main` and the active session branch are at the same commit (`f0c40d4`).

**Governance layer — now fully synced with code (2026-06-24):** SQLite tables: `manufacturers`, `product_manufacturers`, `score_change_logs`, `product_scores`. Service: `scoreAudit.js` logs every score change with paying-client flag. Admin routes at `/api/admin/*` (requireAdmin middleware, checks `users.role`), with real admin UI at `/admin/conflict-log` and `/admin/manufacturers` — see [[Governance Frontend]]. **Public `/transparency` page is live** (panel seats, commitments, live stats from `/api/transparency/stats`) — closes Phase 1 activation step 3. **B2B Digital Product Passport API** at `/v1/passport/:productId`, `/v1/portfolio/score`, `/v1/audit/:productId` for EU ESPR/SGX Scope 3 reporting — see [[Digital Product Passport API]]. `METHODOLOGY.md` (repo root) carries the full GreenGrade v3.0 spec for external readers. **Charter drafted:** `/GreenGrade_Governance_Charter.md` — 3-seat advisory panel (academic, regulatory, non-client industry), 4 powers (methodology audit, score challenge, conflict flag, annual report), conflict-of-interest firewall, voluntary service. **Remaining gaps:** no founding Panel candidates identified yet (panel seats still show "In formation"); no version-controlled changelog for GreenGrade *parameter* changes (distinct from the existing per-product score-audit trail). Stack migration plan at [[Stack Migration Plan]].

**Cold start UX (2026-06-07):** `ApiReadyGate` polls `/api/health` for up to 60s on app load to mask Render free-tier backend spin-up — see [[API Cold Start Gate]].
