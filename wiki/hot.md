...
**Last updated:** 2026-06-28 after CONTRIBUTING.md added.

**Since 2026-05-29 (not previously logged here, recovered from `git log`):** PR #32 added trailingSlash fix for static-hosting 404s and a README rewrite for current stack/governance. PR #33 added the Digital Product Passport API (`GET /api/v1/passport/:productId`, `POST /api/v1/portfolio/score`, `GET /api/v1/audit/:productId` — see `backend/src/routes/passport.js`), `METHODOLOGY.md`, Swagger updates, and a cold-start UX component (`frontend/src/components/ApiReadyGate.js`) that shows a loading state while Render's free-tier backend wakes up. No open PRs as of 2026-06-28.
...
**Active branch:** `claude/nifty-goodall-vhulec` (no open PR as of 2026-06-28; `claude/improve-application-S5njo` was merged via PR #33).
