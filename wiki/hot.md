...
**Last updated:** 2026-08-01 — admin evidence review UI + transparency evidence sources (commit `d0da8f3` on `claude/nifty-goodall-mp449f`).
...
**Open PRs (not yet merged):** None — all prior PRs merged. Active dev on `claude/nifty-goodall-mp449f` (unpushed branch pending PR).

**Latest work (2026-08-01, commit `d0da8f3`):**
- `frontend/src/app/admin/evidence/page.js` — admin UI at `/admin/evidence` for reviewing pending community evidence (approve/reject + notes); completes the community evidence workflow (submission UI in EvidenceSection, backend in admin.js already existed)
- `frontend/src/services/admin.js` — `fetchPendingEvidence`, `reviewEvidence`, `fetchEvidenceSources` added
- `frontend/src/app/transparency/page.js` — "Data Sources & Evidence Registry" section showing canonical evidence sources from `/api/v1/evidence/sources` with reliability badges and DOI links
