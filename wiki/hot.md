...
**Last updated:** 2026-07-12 after user account settings feature.
...
**Current test status:** 153 backend tests passing. Frontend builds 567 static pages (17 routes + 550 product pages).

**Active branch:** `claude/dreamy-dirac-2hy99p` — pushed, PR pending.
...
**User account settings (2026-07-12):** Added `PUT /api/auth/profile` (update display name), `PUT /api/auth/password` (change password with current-password verification), `DELETE /api/auth/account` (delete account + cascade-delete all user data). Auth rate limiter skips in test mode. 16 new integration tests. Frontend `/profile` page: account info, name update, password change, danger-zone account deletion. `AuthContext.updateUser()` syncs name changes to localStorage. Navbar username is now a clickable link to `/profile`.
