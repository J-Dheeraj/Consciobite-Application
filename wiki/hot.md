...
**Last updated:** 2026-07-17 — UX polish and API consistency improvements on `claude/dreamy-dirac-93jpkj`.
...
**Current test status:** 137 backend tests passing (20 added by governance). Frontend builds 566 static pages (16 routes + 550 product pages). All lint/Prettier checks clean.

**Active branch:** `claude/dreamy-dirac-93jpkj` — new branch off merged main. Previous branch `claude/improve-application-S5njo` was merged via PRs #29–#33.

**2026-07-17 improvements landed (commit 1a31650):**
- **Carbon zero-state UX:** When authenticated user has no logs, shows onboarding card with Browse Products / Scan Barcode CTA links. Tip text now only shown once data exists.
- **Admin conflict log pagination:** `getConflictLog()` now returns `{ rows, total }` (was array). Route returns `total`, `limit`, `offset` alongside `logs`. Frontend paginates at 50 rows/page with Prev/Next controls; filter change resets page to 0.
- **Passport route alias:** DPP routes now mounted at both `/api/v1/` and `/api/` so `GET /api/passport/:id`, `POST /api/portfolio/score`, `GET /api/audit/:id` work at both prefixes.
