...
**Last updated:** 2026-07-27 after backend-persisted favorites session.
...
**Current test status:** 150 backend tests passing (7 suites; passport.test.js is on PR #34 branch not yet merged). Frontend builds 566 static pages (16 routes + 550 product pages).
...
**Active branch:** `claude/nifty-goodall-dm8aut` — pushed 2026-07-27, awaiting PR creation.

**Backend-persisted favorites (2026-07-27):** `GET/POST/DELETE /api/favorites` routes live on `claude/nifty-goodall-dm8aut`. Migration `003_user_favorites.sql` creates `user_favorites(user_id, product_id, UNIQUE)` table with cascade delete. Frontend uses React Query to fetch from API when authenticated; localStorage fallback for guests. ProductCard and ProductDetailClient fire-and-forget API calls on toggle. 13 new integration tests (150 total).
