...
**Last updated:** 2026-07-12 after passport test coverage session.
...
**Current test status:** 162 backend tests passing (7 suites). Frontend builds 566 static pages (16 routes + 550 product pages).

**Active branch:** `claude/nifty-goodall-i8sgrw` — pushed, PR not yet opened.

**Session 2026-07-12 work:** Added `backend/__tests__/passport.test.js` (25 tests) for the Digital Product Passport API (`/api/v1/passport/:id`, `/api/v1/portfolio/score`, `/api/v1/audit/:id`). Exposed `fetchPassport()` in frontend services. Added "Download Digital Product Passport (JSON)" button to product detail page (Stats for Nerds section).
