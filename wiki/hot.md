...
**Last updated:** 2026-07-25 after Eco Leaderboard + Carbon CSV Export session.
...
**Current test status:** 137 backend tests passing (note: passport tests on separate branch PR #34). Frontend builds 567 static pages (17 routes + 550 product pages).
...
**Eco Leaderboard + Carbon CSV Export (2026-07-25):** New `/leaderboard` page showing top 10 products overall + top 5 per category, linked to product detail pages. CSV export button added to Carbon Tracker — paginates through all logs and downloads timestamped file. `fetchCarbonLogs()` service updated to accept `{ page, limit }` object. Leaderboard nav link added to Navbar. Branch: `claude/dreamy-dirac-uylhum`.

**Open PRs (as of 2026-07-25):**
- PR #34: DPP frontend (branch `claude/dreamy-dirac-fzmsdt`) — needs CI fix
- PR #36: Product recommendations + Similar Products (branch `claude/dreamy-dirac-4ua0hn`)
- PR #37: npm audit fixes + tier filter + carbon dashboard widget (branch `claude/dreamy-dirac-6st5cb`)

**Active branch:** `claude/dreamy-dirac-uylhum` — pushed, PR not yet created.
