...
**Last updated:** 2026-08-18 — admin evidence review queue UI added (`claude/dreamy-dirac-p0yvea`, commit `aaffcbc`); community evidence workflow now complete end-to-end.
...
**Open PRs (not yet merged):** none (all previous PRs merged). Branch `claude/dreamy-dirac-p0yvea` has the admin evidence review UI — ready for PR.

**Admin evidence review (2026-08-18, commit `aaffcbc`):** `/admin/evidence` page completes the community evidence workflow. Pending submissions listed with citation, source type, year, URL, submitter, expandable methodology, and Approve/Reject action with optional reviewer notes. `fetchPendingEvidence()` and `reviewEvidence()` added to admin service. 10 new backend tests (259 total).
