...
**Last updated:** 2026-06-12 after passport tests + score history session.
...
**Current test status:** 158 backend tests passing (21 new passport tests added 2026-06-12). Frontend builds 566 static pages (16 routes + 550 product pages).

**Active branch:** `claude/dreamy-dirac-tr8nr4` — in progress.

**Passport API (2026-05-29, tested 2026-06-12):** Three endpoints under `/api/v1/` — `GET /passport/:id` (Digital Product Passport), `POST /portfolio/score` (bulk portfolio scoring), `GET /audit/:id` (score change history). All fully tested (21 tests). Frontend connects audit trail to product detail page via lazy-loaded "Score History" toggle using React Query. `fetchProductAudit` and `fetchPassport` are now exported from `frontend/src/services/products.js`.
