...
**Last updated:** 2026-06-07 after quality pass session.
...
**Current test status:** 142 backend tests passing. Frontend builds static pages (16+ routes + 550 product pages). Frontend lint: clean.

**Active branch:** `claude/nifty-goodall-TeYkV` (pushed, no open PR yet).

**Governance layer (complete as of 2026-05-29):** SQLite tables: `manufacturers`, `product_manufacturers`, `score_change_logs`, `product_scores`. Service: `scoreAudit.js`. Admin routes at `/api/admin/*` (requireAdmin). Transparency page at `/transparency` (public, shows score-change stats). Charter at `/GreenGrade_Governance_Charter.md`. Stack migration plan at [[Stack Migration Plan]].

**Quality pass (2026-06-07):** Fixed Footer "Products" link (was `/`, now `/products`). Added `/methodology` to Footer Resources. Added 5 integration tests for `GET /api/transparency/stats` and `GET /api/methodology`. Created `CONTRIBUTING.md`. Test count: 142.
