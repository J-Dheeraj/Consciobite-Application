...
**Tier filter + dashboard widget (PR #37, branch `claude/dreamy-dirac-6st5cb`):** `?tier=green|amber|red` on `GET /api/products` (green ≥7, amber 4–7, red <4) with pill buttons on the products page; authenticated dashboard shows a "Your Carbon Footprint" card (weekly progress vs `WEEKLY_CARBON_GOAL_KG`, total/monthly/log-count). The PR's original `npm audit fix` lockfile change was **dropped as superseded** — main removed swagger-jsdoc entirely.
