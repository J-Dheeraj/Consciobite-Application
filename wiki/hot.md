...
**Last updated:** 2026-07-01 after DPP test coverage session.
...
**Current test status:** 162 backend tests passing (+25 new DPP tests). Frontend builds 566 static pages (16 routes + 550 product pages).

**DPP test coverage (2026-07-01):** Added `backend/__tests__/passport.test.js` (25 tests) covering all 3 Digital Product Passport endpoints: `GET /api/v1/passport/:productId`, `POST /api/v1/portfolio/score`, `GET /api/v1/audit/:productId`. Tests verify response shape, emission breakdown keys, ISO 8601 timestamps, validation errors, 404s, portfolio summary fields, and category benchmarks. Also removed two dead constants (`DEFAULT_PORT` in `index.js`, `DIM` in `greengrade.js`) to cut ESLint warning count from 4 → 2.

**Active branch:** `claude/nifty-goodall-kdko3m` — pushed, no open PR yet.
