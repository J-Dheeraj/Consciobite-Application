...
**Last updated:** 2026-08-15 — catalog export API added (branch `claude/dreamy-dirac-xp4g5d`, commit `2561fea`); all prior PRs merged (no open PRs).
...
**Open PRs:** None — all previous PRs (#34–#47) are merged. Current work is on branch `claude/dreamy-dirac-xp4g5d`.
...
**Test status:** 189+12=201 backend (12 new export tests) + 9 frontend, all expected to pass. Frontend builds 1119 static pages (550 product + 550 passport + routes).

**Catalog Export (2026-08-15, commit `2561fea`):** `GET /api/v1/export/catalog` — full 550-product catalog with GreenGrade scores in JSON (default) or CSV. Query params: `?format=json|csv`, `?category=<name>`, `?tier=green|amber|red`. Returns `X-Catalog-Sha256` (SHA-256 of products.json) and `X-Methodology-Version` headers. Rows pre-computed once at module load (static catalog). Frontend transparency page has JSON + CSV download buttons. Swagger documented under "Catalog Export" tag. Directly addresses the architecture reviewer's "repeatable exports" steer.
