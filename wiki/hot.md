...
**Last updated:** 2026-06-02 after admin UX session.
...
**Active branch:** `claude/dreamy-dirac-uU2Rj` — pushed, no open PR yet.
...
**Admin UX (2026-06-02):** Role field added to login + /me responses (`users.role` column from governance migration). Admin landing page at `/admin` with stats cards + governance checklist. Shared admin layout with tab sub-nav (Overview / Score Audit / Manufacturers). Navbar shows "Admin" link only for `user.role === 'admin'` users. All 137 tests green.
