...
**Last updated:** 2026-08-20 — admin dashboard + evidence review UI landed on `claude/nifty-goodall-vx6knt`; all prior PRs (#34–#47) merged to main.
...
**Open PRs:** all prior PRs merged (#34–#47). Branch `claude/nifty-goodall-vx6knt` has new admin work (see below), not yet PR'd.
...
**Admin dashboard (2026-08-20, branch `claude/nifty-goodall-vx6knt`):** `/admin` landing page linking to all three admin tools; `/admin/evidence` page for reviewing pending community submissions with approve/reject + optional reviewer notes; `GET /auth/me` now returns `role` field so frontend can detect admins; Navbar shows an Admin link (desktop + mobile) when `user.role === 'admin'`; `fetchPendingEvidence()` + `reviewEvidence()` added to admin service.
