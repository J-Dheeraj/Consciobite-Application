...
**Last updated:** 2026-06-22 after scoring changelog feature session.

**Since 2026-05-29 (not previously logged here):** `claude/improve-application-S5njo` shipped a Digital Product Passport API (`backend/src/routes/passport.js` — `/v1/passport/:productId`, `/v1/portfolio/score`, `/v1/audit/:productId` for EU ESPR / SGX Scope 3 reporting), a full `METHODOLOGY.md` technical spec, a B2B-framed README rewrite, and an `ApiReadyGate` cold-start loading component for the Render free tier. Merged via PR #32/#33. Backend test count is now 138 (was 117).

**2026-06-22 session — Governance Phase 2, item 2 (Scoring Changelog):** The governance domain page listed 4 pending Phase 2 transparency items; methodology/transparency pages already covered items 1 and partially 3, so this session closed item 2. Added `METHODOLOGY_CHANGELOG` to `backend/src/services/dataProvenance.js`, exposed via the existing `GET /api/methodology` endpoint's new `changelog` field (mirrors the version-history table in `METHODOLOGY.md` section 11 — note it currently jumps v1.0 → v3.0, no v2.0 entry exists in source docs). Rendered as a new "Version History" section on `/methodology`. Added Supertest coverage in `api.test.js`, Swagger schema update. Board disclosure page (item 3) remains blocked on identifying real founding Panel members — `/transparency` panel seats are still "In formation" placeholders, not a code task.
