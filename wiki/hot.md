...
**Last updated:** 2026-08-14 — portfolio CSV export + admin evidence review UI landed on `claude/dreamy-dirac-20jld2`.
...
**Open PRs (as of 2026-08-14):** None — all previous work merged. Branch `claude/dreamy-dirac-20jld2` has the portfolio export + admin evidence UI commit `e307609` (pushed, PR not yet opened).
...
**Portfolio CSV export (2026-08-14, commit `e307609`):** `GET /api/v1/portfolio/export?ids=1,2,3` — RFC 4180 CSV of Digital Product Passports for up to 100 SKUs; 18 columns (all 7 emission dimensions, GreenGrade score, confidence tier, methodology version). Swagger docs updated. Compare page now has "Export CSV" button that triggers browser file download. `downloadPortfolioExport()` in products service, exported from api barrel.

**Admin Evidence Review UI (2026-08-14, commit `e307609`):** `/admin/evidence` page lists pending community submissions with Approve/Reject buttons and optional reviewer notes. Uses existing backend endpoints `GET /admin/pending-evidence` and `POST /admin/evidence/:id/review`. `fetchPendingEvidence()` + `reviewEvidence()` added to admin service and api barrel.

**Test status:** 257 backend + 9 frontend, all passing. Frontend builds 1119 static pages (550 product + 550 passport + routes).
