...
**Last updated:** 2026-07-05 after passport tests + DPP download button.
...
**Current test status:** 158 backend tests passing (7 suites). Frontend builds 566 static pages (16 routes + 550 product pages).

**Active branch:** `claude/dreamy-dirac-16jwpe` — pushed, no PR open yet.

**Digital Product Passport (2026-07-05):** API routes `/api/v1/passport/:id`, `/api/v1/portfolio/score`, `/api/v1/audit/:id` now covered by 21 integration tests (`backend/__tests__/passport.test.js`). Product detail page has a "Download Digital Passport" button (`fetchPassport()` in `services/products.js`) that fetches the passport JSON and triggers a file download.
