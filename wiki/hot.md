...
**Last updated:** 2026-06-13 after compare page fix and score history UI.
...
**Active branch:** `claude/dreamy-dirac-wmcz27` — dev branch.
...
**Compare page fix (2026-06-13):** Rewrote to use React Query + debounced server-side search. Was capped at 100/550 products and used raw `useEffect`. Now shows top 50 by grade on open, server-filtered on search.

**Score History UI (2026-06-13):** Per-product audit trail surfaced in product detail page. Collapsible "Score History" section lazy-loads from `GET /api/v1/audit/:productId`. New `fetchProductAuditHistory()` service function added to `products.js` and re-exported via `api.js`.
