...
**Last updated:** 2026-08-24 — portfolio scoring page built on `claude/dreamy-dirac-iq8m76`; all previous PRs (#34–#48) merged into main.
...
**Open PRs:** None — all PRs through #48 are merged into main. Branch `claude/dreamy-dirac-iq8m76` has one commit ahead: portfolio scoring UI (commit `44acbc5`).
...
**Test status:** 249 backend + 9 frontend, all passing. Frontend builds 1119+ static pages (550 product + 550 passport + routes including new /portfolio).

**Portfolio scoring UI (2026-08-24, branch `claude/dreamy-dirac-iq8m76`):** `/portfolio` page exposes `POST /api/v1/portfolio/score` (already existed). Users search/select up to 100 SKUs; results: avg GreenGrade KPI cards, score distribution bar chart (green/amber/red), category benchmark table, sortable product table with Passport deep-links. "Portfolio" added to Navbar between Compare and Dashboard.
