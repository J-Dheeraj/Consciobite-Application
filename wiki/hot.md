...
**Last updated:** 2026-07-03 after passport tests + products page React Query migration.
...
**Current test status:** 157 backend tests passing (117 prior + 20 new passport route tests). Frontend builds 566 static pages (16 routes + 550 product pages).

**Active branch:** `claude/nifty-goodall-xp779m` — pushed, no PR yet.

**B2B API (2026-06-08, PR #33):** Digital Product Passport routes mounted at `/api/v1/` — `GET /passport/:id`, `POST /portfolio/score`, `GET /audit/:id`. EU ESPR + SGX Scope 3 reporting. `ApiReadyGate` component handles cold-start loading on free-tier Render. METHODOLOGY.md and updated README added.
