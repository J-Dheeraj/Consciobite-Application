...
**Last updated:** 2026-07-18 after API completeness session.
...
**Current test status:** 174 backend tests passing (+57 since last session). Frontend builds 566 static pages (16 routes + 550 product pages).

**Active branch:** `claude/dreamy-dirac-2eajab` — commit 69b6793 pushed, PR not yet opened.

**New endpoints (2026-07-18):**
- `PATCH /api/carbon/log/:id` — update quantity of own carbon log; clamps 0.1–100, ownership-checked
- `PATCH /api/auth/me` — update name and/or password; password change requires currentPassword verification
- `GET /api/auth/me` now has a logged-in test; logout, refresh, and csrf tests added
