...
**Last updated:** 2026-07-04 after scoring changelog implementation (Phase 2 governance).
...
**Current test status:** 142 backend tests passing. Frontend builds 566 static pages (16 routes + 550 product pages). ESLint and Prettier clean.
...
**Scoring changelog (2026-07-04):** Phase 2 governance — `getMethodologyChangelog()` added to `dataProvenance.js` with v1/v2/v3 entries; `GET /api/methodology/changelog` endpoint exposed; `/methodology` page now renders a collapsible Version History section (current version open, deprecated collapsed). 5 new tests added (142 total passing). All Phase 2 transparency features now complete. Branch: `claude/nifty-goodall-w12iez`.
