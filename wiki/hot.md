...
**Last updated:** 2026-05-31 after admin navigation + role exposure session.
...
**Current test status:** 138 backend tests passing. Frontend builds 566 static pages (16 routes + 550 product pages).

**Active branch:** `claude/nifty-goodall-oxx7j`.

**Governance layer (complete as of 2026-05-29):** SQLite tables: `manufacturers`, `product_manufacturers`, `score_change_logs`, `product_scores`. Service: `scoreAudit.js`. Admin routes at `/api/admin/*`. Frontend pages: `/admin/conflict-log`, `/admin/manufacturers`, `/transparency`. Charter at `/GreenGrade_Governance_Charter.md`.

**Admin navigation fix (2026-05-31):** Auth routes (`/login`, `/me`, `/register`) now return `role` field in user object. Navbar conditionally shows "Audit Log" and "Manufacturers" admin links when `user.role === 'admin'`. Admin pages were previously orphaned (no nav links). CONTRIBUTING.md created.
