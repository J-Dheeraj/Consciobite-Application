...
**Last updated:** 2026-08-04 — Portfolio Scorer frontend page added (`/portfolio`); all open PRs merged (main at `1950915`); no open PRs.
...
**Open PRs:** None. All PRs (#34–#47) merged into main as of 2026-07-31. Branch `claude/nifty-goodall-oxfs41` has one commit ahead of main: portfolio scorer page (commit `533377a`, 2026-08-04).

**Passport frontend (PR #34, merged):** `/passport/[id]` page with `PassportCard` (SVG score ring, 7-dimension emission bars, confidence badge, methodology version); `fetchPassport`/`fetchPortfolioScore`/`fetchAuditLog` in the products service; "Eco Passport" button on the product detail page; 550 pages via `generateStaticParams()`.

**Portfolio Scorer (`claude/nifty-goodall-oxfs41`, commit `533377a`):** `/portfolio` page wiring `POST /api/v1/portfolio/score` — product search/multi-select (up to 100 SKUs), portfolio summary card (avg score, best/worst), category benchmarks with score bars, per-product table sorted by score with passport links. Added to Navbar. 6 new frontend tests.
...
**Test status:** 249 backend + 15 frontend, all passing. Frontend builds 1119 static pages (550 product + 550 passport + routes). Portfolio page is dynamic (no static params needed).
