...
**Last updated:** 2026-08-19 — admin dashboard + evidence review UI + role in /me (branch `claude/nifty-goodall-lipww2`, commit `ab1e05f`).
...
**Admin UI (2026-08-19):** `/admin/page.js` (dashboard linking conflict-log, manufacturers, evidence review); `/admin/evidence/page.js` (approve/reject pending community evidence); `user.role` now returned by `GET /api/auth/me`; Navbar shows "Admin" link for `role === 'admin'` users.
