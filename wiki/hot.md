...
**Last updated:** 2026-08-22 — controlled score publication workflow landed on `claude/dreamy-dirac-64wevh` (commit `21c0823`). Evidence source registry and all previous PRs merged to main.
...
**Open PRs:** None — all previous PRs (#34–#48) merged. New work on `claude/dreamy-dirac-64wevh` (score publication workflow, not yet PR'd).

**Score publication workflow (2026-08-22, commit `21c0823`):** Addresses architecture reviewer's "controlled score publication" steer. `POST /api/admin/rescore?mode=stage` queues changes in `pending_score_changes` (migration 008) without touching live scores. Admins review via `GET /api/admin/pending-scores`, then `POST /api/admin/pending-scores/:id/publish` (applies to live + writes audit) or `POST /api/admin/pending-scores/:id/reject` (discards silently). Frontend: `/admin/pending-scores` page with table + publish/reject modal + admin nav tabs. Swagger: `PendingScoreChange` schema + `Score Publication` tag. 11 new tests (260 total).
...
**Test status:** 260 backend + 9 frontend, all passing. Frontend builds 1121 static pages (550 product + 550 passport + routes including /admin/pending-scores).
