...
**Last updated:** 2026-07-02 after passport test suite and CONTRIBUTING.md.
...
**Current test status:** 162 backend tests passing (added 25-test suite for Digital Product Passport API). Frontend builds 566 static pages (16 routes + 550 product pages).

**Active branch:** `claude/dreamy-dirac-ieicfr` — pushed, PR pending.

**Digital Product Passport API (2026-06-07, merged):** B2B endpoints at `/api/v1/passport/:id`, `/api/v1/portfolio/score`, `/api/v1/audit/:id` for EU ESPR / SGX Scope 3 reporting. `METHODOLOGY.md` added to repo root. `ApiReadyGate` component handles cold-start loading on Render free tier.

**Session 2026-07-02:**
- `backend/__tests__/passport.test.js` — 25 Supertest integration tests covering all 3 passport routes (was missing, violating CLAUDE.md convention)
- `CONTRIBUTING.md` — created at repo root (was listed as planned-but-missing)
