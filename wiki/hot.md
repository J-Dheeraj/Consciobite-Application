...
**Last updated:** 2026-08-05 — admin pending-evidence review page shipped (branch `claude/dreamy-dirac-rdn9ce`, commit `a8daad4`); all previous PRs merged; no open PRs.
...
**Open PRs:** None. All previous PRs (#34, #36, #37, #38, #39, #41, #44, #45, #46, #47) are merged. Current branch `claude/dreamy-dirac-rdn9ce` has the pending-evidence admin UI (not yet PR'd).
...
**Admin UI coverage (2026-08-05):** `/admin/pending-evidence` page added — lists pending community evidence submissions with Approve/Reject buttons and optional reviewer notes; cards disappear on review via React Query invalidation. Service functions `fetchPendingEvidence()` and `reviewEvidence()` added to `frontend/src/services/admin.js` and re-exported through `api.js`. This closes the loop on PR #45's backend routes which had no frontend. All three admin pages: `/admin/conflict-log`, `/admin/manufacturers`, `/admin/pending-evidence`.
