...
**Last updated:** 2026-06-16 after Session 3 passport tests + scoring changelog.
...
**Current test status:** 158 backend tests passing (up from 117 at start, 137 after PR #33). Frontend builds 566 static pages (16 routes + 550 product pages).

**Active branch:** `claude/dreamy-dirac-1wayiq`

**Governance layer (2026-06-16 — Session 3 complete):**
- Session 1: SQLite governance tables, `scoreAudit.js`, admin routes at `/api/admin/*`, `requireAdmin` middleware
- Session 2 (PR #33): Digital Product Passport API (`/api/v1/passport/:id`, `/api/v1/portfolio/score`, `/api/v1/audit/:id`), `ApiReadyGate` cold-start UX, methodology docs
- Session 3 (2026-06-16): Passport route tests (17 new), scoring changelog in `getMethodology()` (v1.0→v3.0 history), changelog rendered on `/methodology` page, tests for `/api/methodology` and `/api/transparency/stats`
- **Advisory panel formation:** still pending (non-code, business work)
- Charter drafted at `/GreenGrade_Governance_Charter.md`
