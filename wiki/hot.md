...
**Last updated:** 2026-06-08 after passport API test coverage.
...
**Current test status:** 162 backend tests passing (7 test suites). Frontend builds 566 static pages (16 routes + 550 product pages).

**Active branch:** `claude/nifty-goodall-RMOAi` (new session branch). Previous work merged via PRs #30-#33 on `claude/improve-application-S5njo`.

**Digital Product Passport API (PR #33, merged):** Routes at `/api/v1/` — `GET /passport/:productId` (full emission breakdown + GreenGrade score), `POST /portfolio/score` (bulk scoring with avg/highest/lowest + category benchmarks), `GET /audit/:productId` (paginated score-change history from SQLite). Test suite added 2026-06-08 (25 tests). `ApiReadyGate` component added to handle Render free-tier cold start (polls `/health` every 3s up to 60s before showing app content).
