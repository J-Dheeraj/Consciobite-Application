...
**Last updated:** 2026-07-24 after User Profile page session.
...
**Current test status:** 137 backend tests passing. Frontend builds 570 static pages (17 routes + 550 product pages + /profile).
...
**User Profile page (2026-07-24):** `/profile` page added. Shows account info (name, email, member-since from `/api/auth/me`), carbon summary (total/weekly emissions + progress bar toward 10 kg weekly goal), favorites count (localStorage), and a quick-link grid. Navbar username (desktop + mobile) is now a `<Link>` to `/profile`. Branch: `claude/dreamy-dirac-pkwtyu` — pushed, PR not yet opened.

**Active branch:** `claude/dreamy-dirac-pkwtyu` — pushed, awaiting PR.
