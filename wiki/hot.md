...
**Last updated:** 2026-07-15 — passport tests + footer fix session.
...
**Current test status:** 157 backend tests passing (7 suites). Frontend builds 566 static pages (16 routes + 550 product pages).

**Active branch:** `claude/dreamy-dirac-h51bda` — pushed, no PR yet.

**Recent session (2026-07-15):**
- Added `backend/__tests__/passport.test.js` — 20 integration tests for Digital Product Passport API (`GET /api/v1/passport/:id`, `POST /api/v1/portfolio/score`, `GET /api/v1/audit/:id`). Backend test count: 137 → 157.
- Fixed broken footer "Products" link (`"/"` → `"/products"`).
- Added `/methodology` link to footer Resources section (page existed but had no nav entry point).
