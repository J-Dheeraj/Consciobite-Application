...
**Last updated:** 2026-06-06 after code quality and coverage session.
...
**Current test status:** 139 backend tests passing. Frontend builds 566 static pages (16 routes + 550 product pages). ESLint clean.

**Active branch:** `claude/dreamy-dirac-3Q4ky`.

**Recent fixes landed (2026-06-06):**
- Products page converted from `useEffect + useCallback` to React Query (`useQuery`, `keepPreviousData`, `staleTime: 60_000`)
- Search debounce added (300 ms) via new `frontend/src/hooks/useDebounce.js` — no API call until typing stops
- Grid dims to 70% opacity during background refetch (smooth pagination UX)
- 2 tests added for `GET /api/transparency/stats` (shape + 550 product count)
