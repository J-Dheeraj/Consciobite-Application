...
**Last updated:** 2026-08-16 — admin evidence review UI shipped (branch `claude/dreamy-dirac-h2lg11`); all prior PRs merged (no open PRs as of 2026-08-16).
...
**Open PRs (not yet merged):** none — all prior PRs (#34–#47) are merged. Active branch: `claude/dreamy-dirac-h2lg11` (admin evidence review UI, commit `8dc3fb6`).
...
**Test status:** 259 backend + 9 frontend, all passing. Frontend builds 1119 static pages (550 product + 550 passport + routes) + new `/admin/evidence` page.

**Admin evidence review (2026-08-16, `claude/dreamy-dirac-h2lg11`):** Closes the community evidence workflow gap. `fetchPendingEvidence()` and `reviewEvidence()` added to admin service; `/admin/evidence` page lists pending submissions with approve/reject UI (notes optional). 10 new backend tests cover GET/POST for pending-evidence admin endpoints, including auth enforcement and idempotency guard.
