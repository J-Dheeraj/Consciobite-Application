...
**Last updated:** 2026-08-03 — bulk catalog export + per-product score history shipped (branch `claude/dreamy-dirac-pba9s4`, commit `33c5695`).
...
**All previous PRs merged:** #34 DPP frontend, #36 recommendations, #37 tier filter, #38 user profile, #39 carbon CSV export, #41 server-side favorites, #44 evidence registry, #45 community evidence submission, #47 README sync — all merged to main. No open PRs.

**Catalog export + score history (2026-08-03, commit `33c5695`, branch `claude/dreamy-dirac-pba9s4`):** `GET /api/products/export?format=csv|json` — bulk download of all 550 product scores with methodology_version + catalog_hash for provenance; CSV default, JSON optional. `GET /api/products/:id/score-history` — public audit trail of score changes per product (date, old/new score, delta, methodology_version, reason; no admin fields exposed). Frontend: `ScoreHistorySection` collapsible component on product detail page (lazy fetch on expand); "Export CSV" button on products listing page. `METHODOLOGY_VERSION` + `CATALOG_HASH` now exported from `scoreAudit.js`. 12 new tests (261 backend total).
