...
**Last updated:** 2026-06-04 after auth role fix and admin navigation.
...
**Current test status:** 144 backend tests passing. Frontend builds 566 static pages (16 routes + 550 product pages).

**2026-06-04 fixes:**
- Auth endpoints (/register, /login, /me) now return `role` in user object — frontend can identify admin users
- Navbar shows admin links ("Audit Log", "Manufacturers" in amber) for `user.role === 'admin'` users
- 7 new backend tests: transparency/stats + auth role fields

**Active branch:** `claude/dreamy-dirac-LvGo0` — pushed, no open PR yet.
