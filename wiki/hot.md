...
**Last updated:** 2026-07-24 after passport tests + profile update endpoint session.
...
**Current test status:** 172 backend tests passing (30 new passport tests + 5 new profile update tests). Frontend builds 566 static pages (16 routes + 550 product pages).
...
**Active branch:** `claude/nifty-goodall-9bzt8o` — pushed, no PR yet.

**New endpoint (2026-07-24):** `PUT /api/auth/profile` — authenticated users can update their display name. Requires `Authorization: Bearer <token>` header. Body: `{ "name": "New Name" }`. Name sanitized, truncated to 50 chars. Uses CSRF protection (bypassed in test env).
